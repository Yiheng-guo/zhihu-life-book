...
当前实现见 [2.0 技术与交付说明](RELEASE_2.0.md)。下方保留 1.3 的历史设计，仅供比较，不代表当前公网能力。

# 1.3 落地路线与技术说明

## 1.3 的选择如何影响下一页

`frontend/branches.mjs` 根据上页行动与来源生成当前处境及可用选项；`story.mjs` 仍校验四阶段的合法推进。阶段 ID 是进度，branch_id 是当时的处境，二者分开。历史保存处境、行动标签、focus、来源和取舍，不根据当前页面反推旧选择。

同一阶段有不同选项集合，后端拒绝从另一条分支提交的选项。选择的来源能增加 follow_source 行动；用户自行决定是否采纳，来源不自动替用户选择。八个毕业情境始终可用，前面行动只改变呈现上下文和优先展示顺序，不预测结果。
...
| scripts/build.mjs | 只将 8 个公开文件复制到 dist |
...
`npm test` 的 10 项测试覆盖来源必选与替换、条件分支、8 种结局、4 节连续性、无效事件与过期版本、会话隔离及 API。`tests/ui-smoke.mjs` 以本地安装的 Playwright 对静态与 API 两种模式实际点击完整流程，检查独立原文链接、来源更换、承接问题、结尾、下载、重新开始和 390px 手机宽度无横向溢出。原文新窗口的网络请求在回归中被拦截，测试验证跳转行为，不代表已逐个验证知乎页面的公网可访问性。
