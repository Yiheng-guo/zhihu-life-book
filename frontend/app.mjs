...
function choiceButton(c,i){
 ...
 const body=element('span');body.append(element('strong',c.label));
 if(c.hint)body.append(element('small',c.hint));
 const trade=element('span',undefined,'choice-trade');
 trade.append(element('span',`${c.situation?'可以争取':'可能得到'} · ${c.gain}`),element('span',`${c.situation?'需要面对':'可能付出'} · ${c.cost}`));
 body.append(trade);b.append(element('span',String.fromCharCode(65+i),'choice-number'),body);
...
function sourceCard(r){
 ...
 input.onchange=()=>send('select',{source_id:r.id});
 head.append(input,element('span',r.type),element('span','','chosen'));
 label.append(head,element('strong',r.short_summary));
 const details=element('details',undefined,'source-details');details.append(element('summary','展开标题与摘要'),element('p',r.title),element('p',r.summary));
 const bottom=element('div',undefined,'source-bottom');bottom.append(element('span',r.author),sourceLink(r));
 card.append(label,details,bottom);return card;
}
...
function render(focus){const v=current;
 ...
 $('chapter').textContent=v.node.label+' · 情境演绎';$('heading').textContent=v.phase==='story'?v.node.title:v.echo.title;
 ...
 $('choices').replaceChildren(...main.map(choiceButton));
 ...
 const selected=v.references.find(r=>r.id===v.source_id);$('selectedReflection').hidden=!selected;if(selected){$('selectedQuestion').textContent=selected.question;$('sourcePreview').textContent=v.source_preview.label+'：'+v.source_preview.text;}
...
function renderEnding(v){
 ...
 $('history').replaceChildren(...v.history.map(h=>{const li=element('li');const a=sourceLink(h.source);a.textContent=`参考：${h.source.title} ↗`;li.append(element('small',h.stage),element('strong',h.choiceLabel),element('small',`当时的取舍：${h.gain} / ${h.cost}`),a);return li;}));window.scrollTo({top:0,behavior:'instant'});
}
