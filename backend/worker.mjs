...
export default {async fetch(request,env,context){
 ...
 if(path==='/healthz')return Response.json({ok:true,version:'2.1',runtime_ai:!!env.ZHIHU_ACCESS_SECRET,storage:!!env.DB});
 ...
 const body=item.base64?Uint8Array.from(atob(item.body),c=>c.charCodeAt(0)):item.body;return new Response(request.method==='HEAD'?null:body,{headers:{'content-type':item.type,'cache-control':'no-cache','x-content-type-options':'nosniff','referrer-policy':'strict-origin-when-cross-origin','permissions-policy':'camera=(), microphone=(), geolocation=()'}});
}};
